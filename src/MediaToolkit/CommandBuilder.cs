...
namespace MediaToolkit
{
    internal class CommandBuilder
    {
        ...
        static string GetMetadata(MediaFile inputFile) => string.Format("-i \"{0}\" ", inputFile.Filename);

        static string GetThumbnail(MediaFile inputFile, MediaFile outputFile, ConversionOptions conversionOptions)
        {
        ...
        static string Convert(MediaFile inputFile, MediaFile outputFile, ConversionOptions conversionOptions)
        {
            var commandBuilder = new StringBuilder();
            ...
            if (conversionOptions.Target != Target.Default)
            {
                ...
                {
                    commandBuilder.AppendFormat(" {0}-{1} \"{2}\" ", 
                ...
                commandBuilder.AppendFormat("{0} \"{1}\" ",
                    conversionOptions.Target.ToString().ToLowerInvariant(),
                    outputFile.Filename);
            ...
            {
                commandBuilder.AppendFormat(" -ab {0}k",
                    conversionOptions.AudioBitRate);
            }

            // Audio sample rate
            if (conversionOptions.AudioSampleRate != AudioSampleRate.Default)
            {
                commandBuilder.AppendFormat(" -ar {0} ",
                    conversionOptions.AudioSampleRate);
            }

            // Maximum video duration
            if (conversionOptions.MaxVideoDuration != null)
            {
                commandBuilder.AppendFormat(" -t {0} ",
                    conversionOptions.MaxVideoDuration);
            }

            // Video bit rate
            if (conversionOptions.VideoBitRate != null)
            {
                commandBuilder.AppendFormat(" -b {0}k ",
                    conversionOptions.VideoBitRate);
            }

            // Video frame rate
            if (conversionOptions.VideoFps != null)
            {
                commandBuilder.AppendFormat(" -r {0} ",
                    conversionOptions.VideoFps);
            }

            // Video size / resolution
            if (conversionOptions.VideoSize == VideoSize.Custom)
            {
                commandBuilder.AppendFormat(" -vf \"scale={0}:{1}\" ",
            ...
            if (conversionOptions.SourceCrop != null)
            {
                ...
                commandBuilder.AppendFormat(" -filter:v \"crop={0}:{1}:{2}:{3}\" ",
                    crop.Width,
                    crop.Height,
                    crop.X,
                    crop.Y);
